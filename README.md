...
## Using the IMAGE Monarch Client
This is the current workflow for pushing experiences onto the Monarch Client.

### Starting the IMAGE Monarch Client
1. Connect the Monarch to a computer through USB.
2. Open the file ClassroomSelector.java in Android Studio
3. change the variable channelSubscribed to the value in Graphic ID field in the Tactile Authoring tool.
As of November 22, 2024, see the README of IMAGE-TactileAuthoring for more details.
4. Select the Monarch from the dropdown list and run the app.

Note that we highly recommend using a utility such as ScreenCopy while running the Monarch Client, especially if permissions need to be granted to the device.

### Classroom Mode
Classroom Mode provides two styles of information organization, these are "Exploration Mode" and "Guidance Mode". These provide two complementary ways of breaking down information in diagrams.

#### Exploration Mode
Exploration mode presents all components of a diagram at once, so that you can familiarize yourself with the entirety of the information.

#### Guidance Mode
Guidance mode presents components of a diagram in sequence, so that you can investigate every item in detail.

### Photo Mode
Send a photo from a connected computer to the Monarch.

### Map Mode
Send a map from a connected computer to the Monarch.
